...
Adapted from Brent Taylor's Python Driver 1.2 btaylor@fuzzylogicstudios.com
"""
...
def SetAxis(JoyState, Axis, Value):
    """
    Helper function to set axis value. ADDED BY THEBAGMASTER

    /param JoyState A joystick State structure where you want to make the button changes.
    /param Axis you wish to change. 0-7 Stated above : XAxis=0,YAxis=1 etc
    /param Current value of axis. -INTMAX16 to INTMAX16
    ...
    if Value > AXIS_MAX:
        ...
        Value = AXIS_MIN
    if Axis == 0:
        JoyState.XAxis = Value
    elif Axis == 1:
        JoyState.YAxis = Value
    elif Axis == 2:
        JoyState.ZAxis = Value
    elif Axis == 3:
        JoyState.XRotation = Value
    elif Axis == 4:
        JoyState.YRotation = Value
    elif Axis == 5:
        JoyState.ZRotation = Value
    elif Axis == 6:
        JoyState.Slider = Value
    elif Axis == 7:
        JoyState.Dial = Value
